"use client"
import { sil_property, granny_flat } from 'config/page'
...
const SlideModalForGranny: React.FC<Type> = ({ close, currentId }) => {
    ...
    const handlePrev = () => {
        const newIndex = (activeIndex - 1 + granny_flat.length) % granny_flat.length;
    ...
    const handleNext = () => {
        const newIndex = (activeIndex + 1) % granny_flat.length;
        setActiveIndex(newIndex);
        if (swiperRef.current && swiperRef.current.swiper) {
            swiperRef.current.swiper.slideTo(newIndex);
        }
                ...
                >
                    <SwiperSlide key={activeIndex}>
                        <div className="flex flex-col  h-full w-full   items-center justify-self-center  rounded-2xl bg-white ">
                            <Image src={granny_flat[activeIndex].image} alt={"slide"}
...
export default SlideModalForGranny