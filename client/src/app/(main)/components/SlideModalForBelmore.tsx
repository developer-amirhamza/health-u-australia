"use client"
import { sil_property, belmore_slides } from 'config/page'
...
const SlideModalForBelmore: React.FC<Type> = ({ close, currentId }) => {
    ...
    const handlePrev = () => {
        const newIndex = (activeIndex - 1 + belmore_slides.length) % belmore_slides.length;
    ...
    const handleNext = () => {
        const newIndex = (activeIndex + 1) % belmore_slides.length;
        setActiveIndex(newIndex);
        if (swiperRef.current && swiperRef.current.swiper) {
            swiperRef.current.swiper.slideTo(newIndex);
        }
                ...
                >
                    <SwiperSlide key={activeIndex}>
                        <div className="flex flex-col  h-full w-full   items-center justify-self-center  rounded-2xl bg-white ">
                            <Image src={belmore_slides[activeIndex].image} alt={"slide"}
...
export default SlideModalForBelmore