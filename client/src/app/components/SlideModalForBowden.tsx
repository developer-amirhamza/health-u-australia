"use client"
import { sil_property, bowden_slides } from 'config/page'
...
const SlideModalForBowden: React.FC<Type> = ({ close, currentId }) => {
    ...
    const handlePrev = () => {
        const newIndex = (activeIndex - 1 + bowden_slides.length) % bowden_slides.length;
    ...
    const handleNext = () => {
        const newIndex = (activeIndex + 1) % bowden_slides.length;
        setActiveIndex(newIndex);
        if (swiperRef.current && swiperRef.current.swiper) {
            swiperRef.current.swiper.slideTo(newIndex);
        }
                ...
                >
                    <SwiperSlide key={activeIndex}>
                        <div className="flex flex-col  h-full w-full   items-center justify-self-center  rounded-2xl bg-white ">
                            <Image src={bowden_slides[activeIndex].image} alt={"slide"}
...
export default SlideModalForBowden